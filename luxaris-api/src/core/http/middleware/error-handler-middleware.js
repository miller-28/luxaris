...
function error_handler(error, req, res, next) {
    ...
    if (node_env === 'development') {
        error_logger.error(`[Request ${request_id}] ${error.message}`);
    }
    ...
    if (error.code && (error.code.startsWith('22') || error.code.startsWith('23') || error.code.startsWith('42'))) {
        ...
        error_logger.error(`[Request ${request_id}] Database error: ${error.code} - ${error.message} | Table: ${error.table || 'N/A'} | Constraint: ${error.constraint || 'N/A'}`);
    ...
    error_logger.error(`[Request ${request_id}] Unexpected error: ${error.message} | Stack: ${error.stack}`);
