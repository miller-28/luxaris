...
export class Channel {
   ...
   */
    get supportedFeatures() {
        const features = [];
        if (this.limits.supports_images) {
            features.push('Images');
        }
        if (this.limits.supports_links) {
            features.push('Links');
        }
        if (this.limits.supports_videos) {
            features.push('Videos');
        }
        if (this.limits.max_text_length) {
            features.push(`Text: ${this.limits.max_text_length} chars`);
        }
        return features;
    }
